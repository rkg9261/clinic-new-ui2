...
const Therapy = () => {
  ...
  /* FIRST VISIT THERAPY ANIMATION */
    
   
  const therapySectionRef = useRef(null);
  const [therapyFirstVisit, setTherapyFirstVisit] = useState(false);
  useEffect(() => {
    const section = therapySectionRef.current;
    if (!section) return;
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          setTherapyFirstVisit(true);
          // Stop observing after first visit
          observer.disconnect();
        }
      },
      {
  ...
  return (
    <section
      ...
      className={`therapy-section ${
        therapyFirstVisit ? "therapy-first-visit" : ""
      }`}
    >
      {/* Heading */}
